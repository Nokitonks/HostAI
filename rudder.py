...
class RRLSTM(nn.Module):
    ...
    def redistribute_reward(self, states, actions, state_mapping):
        # Prepare LSTM inputs
    ...
    def train(self, episode,state_mapping):
        ...
        while loss_average > 0.25:
            ...
            main_loss = all_timestep_loss[range(self.lstm_batch_size), length[:] - 1].mean()
...
class LessonBuffer:
    ...
    def different_returns_encountered(self):
        if self.buffer_is_full:
            reward_sums = np.sum(self.rewards_buffer,
                                 axis=1)  # Sum along the reward sequence dimension (second dimension)
        else:
            reward_sums = np.sum(self.rewards_buffer[:self.next_spot_to_add], axis=1)

        return np.unique(reward_sums).shape[0] != reward_sums.shape[0]
    ...
    def add(self, states, actions, rewards):
        ...
        self.states_buffer[next_ind, :trial_length] = states.squeeze()
        self.states_buffer[next_ind, trial_length:] = 0
        self.actions_buffer[next_ind, :trial_length ] = actions
        self.actions_buffer[next_ind, trial_length:] = 0
        self.rewards_buffer[next_ind, :trial_length ] = rewards
        self.rewards_buffer[next_ind, trial_length:] = 0
        self.lens_buffer[next_ind] =trial_length
